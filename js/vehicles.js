...
function openForm(item = null) {
    ...
    form.onsubmit = e => {
        ...
        data.vehicles = items; SM.save(data); closeForm(); render(document.getElementById("search")?.value || ""); SM.toast("Saved successfully");
    };
}
function closeForm() { document.getElementById("modal")?.classList.remove("show") }
function editItem(id) { openForm(items.find(x => x.id === id)) }
function deleteItem(id) { if (confirm("Delete this record?")) { items = items.filter(x => x.id !== id); data.vehicles = items; SM.save(data); render(); SM.toast("Deleted successfully") } }
render();