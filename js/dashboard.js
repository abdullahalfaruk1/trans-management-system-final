...
function row(x) {
    ...
    return vals.map((v, i) => `<td>${i === vals.length - 1 && "bookings" !== "routes" && "bookings" !== "fuel" && "bookings" !== "maintenance" && "bookings" !== "expenses" ? SM.badge(v) : SM.esc(v)}</td>`).join("");
}
window._rowVals = function (x) {
    return [x.passenger, x.trip, x.seat, '৳' + Number(x.fare).toLocaleString(), x.status];
};
function openForm(item = null) {
    ...
    form.onsubmit = e => {
        ...
        obj.status = item?.status || 'Confirmed';
        if (item) items = items.map(x => x.id === item.id ? obj : x); else items.push(obj);
        data.bookings = items; SM.save(data); closeForm(); render(document.getElementById("search")?.value || ""); SM.toast("Saved successfully");
    };
}
function closeForm() { document.getElementById("modal")?.classList.remove("show") }
function editItem(id) { openForm(items.find(x => x.id === id)) }
function deleteItem(id) { if (confirm("Delete this record?")) { items = items.filter(x => x.id !== id); data.bookings = items; SM.save(data); render(); SM.toast("Deleted successfully") } }
render();