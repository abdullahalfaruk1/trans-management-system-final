...
function openForm(item = null) {
    ...
    form.onsubmit = e => {
        ...
        obj.status = item?.status || 'Scheduled';




        if (item) items = items.map(x => x.id === item.id ? obj : x); else items.push(obj);
        data.trips = items; SM.save(data); closeForm(); render(document.getElementById("search")?.value || ""); SM.toast("Saved successfully");
    };
}
function closeForm() { document.getElementById("modal")?.classList.remove("show") }
function editItem(id) { openForm(items.find(x => x.id === id)) }
function deleteItem(id) { if (confirm("Delete this record?")) { items = items.filter(x => x.id !== id); data.trips = items; SM.save(data); render(); SM.toast("Deleted successfully") } }
render();